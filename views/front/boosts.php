...
if(isset($_POST["payer"]))
{
    ...
    try
    {  
        $theBoost = $boostc->rechercherBoostID($_POST['id']);
    $mail->SMTPDebug = 0;
    $mail->isSMTP();
    ...
    $mail->Subject = 'Thank you for your purchase '.$_POST["nom"].'';
    $message = file_get_contents('receiptmailBoost.phtml'); 
    // VARIABLES
        $message = str_replace('%name%',$_POST["nom"],$message);
        $message = str_replace('%eventtitle%',$theBoost['type'],$message);
        $message = str_replace('%reservationID%',$theBoost['id'],$message);
        $message = str_replace('%boostsamount%',$theBoost['nombre'],$message);
        $message = str_replace('%total%',$theBoost['prixtotal'],$message);
    // VARIABLES
                ...
                <div class="col-lg-4">
                    <aside class="sidebar widget-area">
                        <div id="search-2" class="widget widget_search">
                            ...
                            <label>
                                <select name="id" class="comboboxc">
                                    <option value="" disabled selected>ID</option>
                                    <?php $listeBoosts=$boostc->afficherBoosts();
                                    foreach($listeBoosts as $Boost){ ?>
                                    <option value="<?php echo $Boost['id']; ?>" ><?php echo $Boost['id']; ?></option>
                                    <?php } ?>
                                </select>     
                        </label>
                            <label>
